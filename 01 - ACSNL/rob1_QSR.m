...
%[prog,T0] = sospolyvar(prog,monomials(e_n,2:4),'wscoeff');
...
S0 = [S10 [0; S20(1); S20(2); S20(3); 0; 0; 0; S20(3); 0; S20(5); S20(6); 0; S20(7); S20(8); S20(9); 0; 0; 0; 0; 0]];
%S0 = [[S10(1); S10(2); S10(3); S10(4); S10(5); 0; 0; 0; 0; 0; 0; 0; 0; 0; S10(6); 0; 0; 0; 0; S10(7)] [0; S20(1); S20(2); S20(3); 0; 0; 0; 0; 0; 0; 0; 0; 0; S20(3); 0; 0; 0; 0; 0; 0]];
...
cond1 = -gradV0*(f+g*u)-s1*g1+h'*Q0*h+2*h'*S0*u+u'*R0*u;
cond2 = V0-beta1*(e1^2+e2^2+e3^2+e4^2+e5^2)^nv;
%cond3 = T0-beta2*(e1^2+e2^2+e3^2+e4^2+e5^2)^nt;
%cond4 = s1-beta*(e1^2+e2^2+e3^2+e4^2+e5^2)^ns;
prog = sosineq(prog, cond1);
prog = sosineq(prog, cond2);
%prog = sosineq(prog, T0);
...
if S0*inv(R0)*S0'-Q0 >=0
   ...
   % [prog,T] = sospolyvar(prog,monomials(e_n,2:4),'wscoeff');
    ...
    cond1 = -gradV*(f+g*u)-s1*g1+h'*Q*h+2*h'*S*u+u'*R*u;
    cond2 = V-beta1*(e1^2+e2^2+e3^2+e4^2+e5^2)^nv;
    %cond3 = T-beta2*(e1^2+e2^2+e3^2+e4^2+e5^2)^nt;
    %cond4 = s1-beta*(e1^2+e2^2+e3^2+e4^2+e5^2)^ns;
    cond5 = R0-R;
