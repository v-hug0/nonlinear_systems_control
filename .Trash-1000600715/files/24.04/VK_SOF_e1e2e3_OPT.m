...
while i<i_max
    ...
    %...
    dV = -gradV*(f+g*(u+ubar))-alfa*(rho-V)-s1*g1-gamma*V; % dV after S-procedure
    %...
    ...
    %...
    V_PD = V-l1;
    dV = -gradV*(f+g*(u+ubar))-alfa*(rho-V)-s1*g1-gamma*V;
    %...
    prog = sosineq(prog,V_PD);
    prog = sosineq(prog,dV);
    % optimization
    prog = sossetobj(prog,-gamma);
    % second solution (V,s1,gamma)
    sol = sossolve(prog);
    %...
    gamma = sosgetsol(sol,gamma)
    s1 = sosgetsol(sol,s1);
